...
3. Zaktualizuj `_MOC.md` kontekstu, jeśli doszły/zniknęły notatki.
   - **Nota UNIWERSALNA (craft użyteczny w ≥2 kontekstach biznesowych):** jeśli sesja utworzyła LUB awansowała notatkę uniwersalną, jej wpis w `_MOC.md` oraz każdy awans emerging→canon należą do MOC PULI BAZOWEJ (`03-Resources/general-business/knowledge/_MOC.md`), NIE do MOC bieżącego kontekstu (test uniwersalna-vs-kontekstowa: `brain-conventions`).
