...
0. **Rozstrzygnij UNIWERSALNA vs SPECJALISTYCZNA** (trzeci wymiar zapisu — test w `brain-conventions`, NIE przepisuj): czy teza to czysty craft użyteczny w ≥2 kontekstach biznesowych, niezależny od tego jednego venture? **TAK →** pisz do bazy uniwersalnej `03-Resources/general-business/knowledge/` (`context: general-business`) + jej `_MOC.md` (spływa do skilli kontekstów dziedziczących przez `inherits`; nota uniwersalna linkuje TYLKO w górę/w bok, nie w dół do noty kontekstowej — silnik zablokuje commit). **NIE →** bieżący kontekst `03-Resources/<ctx>/knowledge/`. Fail-safe: domyślaj się kontekstu specyficznego; promuj na realny popyt 2. kontekstu. (Ustala `context:` + docelowy `dir`+`_MOC` dla kroków 1–2.)
