...
1. Utwórz/rozszerz notatkę `03-Resources/<ctx>/knowledge/<slug>.md` **BEZPOŚREDNIO** (plik vaulta — NIE plik-artefakt, więc NIE przez ai-manager-agent dla samej NOTATKI), schemat wyżej.
2. Zaktualizuj `_MOC.md` **BEZPOŚREDNIO** pod właściwą sekcją — **WYMAGANE ZAWSZE** (silnik flaguje sierotę, gdy notatka nie jest referencjonowana przez żaden skill ANI nie ma jej w `_MOC`; wpis w MOC to JEDYNY orphan-guard dla notatek brain-only).
